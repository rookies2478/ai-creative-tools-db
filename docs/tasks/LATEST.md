...
- preexisting_untracked_files:
  ...
  - prod_check.html
- current_active_task: none（add-reverse-links-and-fix-category-wordingはcompletedへ移動）
- latest_completed_task: docs/tasks/completed/2026-07-29-add-reverse-links-and-fix-category-wording.md（結果: 独自動画活用監査タスクの結果を受け、Candidate F（逆リンク補完）とCandidate A（カテゴリページ文言修正）のみ実施。実装前の再調査で、runway/pika/luma-aiは既にsrc/data/toolRelatedLinks.ts経由で比較記事・conditions/freeへのリンクを保持済みと判明し、先行監査の「4ツール全欠落」という前提を訂正。実際の欠落はkling-aiのconditionTagsにおける/conditions/free/リンクのみだったため、同エントリへ{ href: '/conditions/free/', label: '無料で使えるAIツール' }を1件追加（4件中4件、表示上限内）。runway/pika/luma-aiは重複回避のため無変更。src/pages/categories/video-generation/index.astroの比較記事カード説明文を「Kling AI・Pika・Runway・Luma AI」の4ツール表記から「Kling AI・Runway・Pika・Luma AI・PixVerse・CapCut AI・Hailuo AI・Vidu AI」の8ツール表記へ更新（1箇所のみ、title/H1/canonical無変更）。poster・新規コンポーネント・新規ページ・generatedVideos.ts・videoFreeDownloadStatus.ts・comparisonEligibleは無変更。build 92ページ PASS、validate:task PASS、validate:scope PASS、git diff --check PASS。dist/への直接アクセスが本環境権限でブロックされたため生成HTML直接確認は未実施（過去タスクと同様の制約）。本番反映なし）
