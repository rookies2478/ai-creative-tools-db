...
- latest_commit: （push後に確定。直前commitはa7dc852 "Update LATEST.md with confirmed commit sha and sync status"）
- branch: master
- origin_sync: push後に確認予定
- working_tree: add-impact-site-verification-meta commit・push完了予定、working tree clean見込み
...
- next_candidate: InVideo AIのアフィリエイトURLをASP（Impact.com）で申請・承認取得しtoolAffiliateLinks.tsへ登録、次28日サイクルでのanalytics rotation --apply実行、vidu-ai/hailuo-ai動画のdist生成HTML実表示確認（権限解除後）、または次のツール動画の登録候補選定
