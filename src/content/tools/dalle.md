...
C2PA（Coalition for Content Provenance and Authenticity）標準の不可視メタデータが生成画像に埋め込まれる場合があります。従来型の可視透かしとは異なりますが、AI生成を示す情報が含まれる場合があります。最新の仕様はOpenAI公式情報でご確認ください。
...
> **免責事項：** このページの情報は公式ソース（[DALL·E 3公式ページ](https://openai.com/dall-e-3)・[利用規約](https://openai.com/policies/usage-policies)・[API料金](https://openai.com/pricing)）をもとに作成していますが、内容は変更される可能性があります。法的助言ではありません。利用前に必ず最新の公式情報をご確認ください。
