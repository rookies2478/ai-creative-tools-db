...
// - home-showcase-v1: トップページ用。比較記事の主材料にしない（comparisonEligible: false 固定）。
...
export const generatedImages: GeneratedImage[] = [
  {
    ...
    promptVersion: 'home-showcase-v1',
    prompt: '（管理者作成・詳細非公開）',
    negativePrompt: '',
    model: 'Adobe Firefly',
    ...
    pageSlug: 'home',
    purpose: 'トップページファーストビュー作例ショーケース用。管理者がAdobe Fireflyで作成した参考作例サンプル。image-tool-benchmark-v1 の比較用画像ではない。',
    usageNote: '本画像は管理者が各ツールで作成したトップページ用の参考作例サンプルです。image-tool-benchmark-v1 の同一条件比較用画像ではありません。生成結果はプロンプト・設定・モデル・タイミングによって変わる場合があります。商用利用条件は公式情報をご確認ください。本記事は法的助言ではありません。',
  },
  {
    ...
    reviewedAt: '2026-06-10',
    pageSlug: 'home',
    purpose: 'トップページファーストビュー作例ショーケース用。管理者がLeonardo AIで作成した参考作例サンプル。image-tool-benchmark-v1 の比較用画像ではない。',
    usageNote: '本画像は管理者が各ツールで作成したトップページ用の参考作例サンプルです。image-tool-benchmark-v1 の同一条件比較用画像ではありません。生成結果はプロンプト・設定・モデル・タイミングによって変わる場合があります。商用利用条件は公式情報をご確認ください。本記事は法的助言ではありません。',
  },
  {
    ...
    reviewedAt: '2026-06-10',
    pageSlug: 'home',
    purpose: 'トップページファーストビュー作例ショーケース用。管理者がStable Diffusionで作成した参考作例サンプル。image-tool-benchmark-v1 の比較用画像ではない。',
    usageNote: '本画像は管理者が各ツールで作成したトップページ用の参考作例サンプルです。image-tool-benchmark-v1 の同一条件比較用画像ではありません。生成結果はプロンプト・設定・モデル・タイミングによって変わる場合があります。商用利用条件は公式情報をご確認ください。本記事は法的助言ではありません。',
  },
  {
